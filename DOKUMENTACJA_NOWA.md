# Nowa Dokumentacja Projektu: Platforma Analityki AI (v4)

Data aktualizacji: 26 września 2025

## 1. Podsumowanie Projektu i Architektury

**Opis:** Aplikacja webowa (`platforma-ai`) typu SPA (Single Page Application) służąca jako platforma SaaS do analityki budowlanej AI. Aplikacja posiada rozbudowany system autoryzacji i ról użytkowników (`super-admin`, `company-admin`, `company-user`). 

### Kluczowe Funkcjonalności:
- **System Firm i Użytkowników:** Aplikacja obsługuje wiele firm, z których każda ma administratora (`company-admin`) i użytkowników (`company-user`).
- **Panel Super-Admina:** Zaawansowany, wielofunkcyjny panel do zarządzania firmami, agentami, analityką i logami, wyposażony w tryb pełnoekranowy dla lepszej analizy danych.
- **Zarządzanie Zespołem:** Administrator firmy może samodzielnie dodawać nowych pracowników, ustawiając dla nich dane i hasło początkowe.
- **Agent Wyszukiwania Podwykonawców:** Funkcjonalność oparta o AI do wyszukiwania i analizy potencjalnych podwykonawców.

### Stos Technologiczny:
- **Frontend:** React, TypeScript, React Router DOM, React-Bootstrap.
- **Backend:** Firebase (Authentication, Firestore, Cloud Functions).
- **Narzędzia:** Create React App (`react-scripts`), Firebase CLI.

### Architektura Backendu (Cloud Functions):
Projekt wykorzystuje architekturę serverless. Kluczowa logika biznesowa, wymagająca podwyższonych uprawnień, jest zamknięta w bezpiecznych **Cloud Functions**.
- **`inviteUser`:** Funkcja typu `onCall`, która pozwala zalogowanemu administratorowi firmy na utworzenie nowego konta pracownika (w Authentication) oraz jego profilu (w Firestore) z odpowiednią rolą i przypisaniem do firmy.
- **`runAgent1_findSubcontractors`:** Funkcja typu `onRequest` z obsługą CORS, która uruchamia proces wyszukiwania podwykonawców.

---

## 2. Kompletna Struktura Projektu

```
/home/flowmistrz/platforma-ai/
├── .firebaserc
├── .gitignore
├── README.md
├── firebase.json
...
## 3. Pełny Kod Źródłowy Wszystkich Plików
...
/**
 * Główna funkcja dla Agenta 1: Wyszukiwanie Podwykonawców.
 * Przyjmuje dane z formularza, przetwarza je w 5 etapach i zapisuje wyniki.
 */
export const runAgent1_findSubcontractors = functions.region("europe-west1").https.onRequest((req, res) => {
  corsHandler(req, res, async () => {
    // Krok 0: Walidacja i uwierzytelnienie
    ...
    if (!specialization || !city) {
      res.status(400).json({ error: "Pola 'specjalizacja' i 'miasto' są wymagane." });
      return;
    }

    const db = admin.firestore();
    const taskRef = db.collection("agent_tasks").doc(); // Tworzymy ID zadania z góry

    // Zapisujemy wstępny status zadania w Firestore
    await taskRef.set({
      ownerUid: uid,
    ...
    try {
      // ETAP 1: Analiza i Rozbudowa Zapytania (AI)
      const expandedQuery = await expandQueryWithAI(specialization);

      // ETAP 2: Wstępne Przeszukanie Wybranych Źródeł
      ...
      res.json({ data: { success: true, taskId: taskRef.id } });

    } catch (error) {
      await taskRef.update({ status: "failed", error: (error as Error).message });
      console.error("Błąd podczas pracy agenta:", error);
      res.status(500).json({ error: "Wystąpił nieoczekiwany błąd podczas pracy agenta." });
    }
  });
});

// --- FUNKCJE POMOCNICZE (SYMULACJE) ---
...
// Funkcja do zapraszania nowego użytkownika do firmy
export const inviteUser = functions.https.onCall(async (data, context) => {
  // 1. Sprawdzamy, czy osoba wywołująca funkcję jest zalogowana.
  if (!context.auth) {
  ...
  const { newUserEmail, newUserName, newUserPassword, companyId } = data;

  // 2. Sprawdzamy, czy osoba wywołująca jest adminem tej firmy.
  ...
  try {
    // 3. Tworzymy nowe konto użytkownika w Firebase Authentication
    const newUserRecord = await admin.auth().createUser({
      email: newUserEmail,
      displayName: newUserName,
      password: newUserPassword, // Używamy hasła z formularza
    });

    // 4. Tworzymy profil nowego użytkownika w bazie Firestore
    await admin.firestore().collection("users").doc(newUserRecord.uid).set({
      uid: newUserRecord.uid,
      email: newUserEmail,
      name: newUserName,
      companyId: companyId,
      role: "company-user", // Domyślna rola dla nowego pracownika
    });

    // W przyszłości tutaj można dodać wysyłkę e-maila z zaproszeniem
    return { success: true, message: `Użytkownik ${newUserEmail} został pomyślnie dodany.` };
  } catch (error: any) {
    console.error("Błąd podczas tworzenia użytkownika:", error);
    // Zwracamy bardziej szczegółowy błąd do klienta, jeśli to możliwe
    if (error.code === 'auth/email-already-exists') {
        throw new functions.https.HttpsError('already-exists', 'Ten adres e-mail jest już zarejestrowany.');
    }
    throw new functions.https.HttpsError(
      "internal",
...
import Agent1FindSubcontractorsPage from './pages/Agent1FindSubcontractorsPage';
      ...
      <main className="py-4">
        <Container>
          <Routes>
            ...
            />
            <Route
              path="/agents/find-subcontractors"
              element={
                <ProtectedRoute roles={['company-admin', 'company-user']}>
                  <Agent1FindSubcontractorsPage />
                </ProtectedRoute>
              }
            />
...
import { AuthUser, UserProfile, Company } from '../type';
        ...
        <Col md={6}>
            <Card>
                <Card.Body>
                    ...
                    <Link to="/agents/find-subcontractors">
